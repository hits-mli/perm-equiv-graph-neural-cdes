...
class ODEDataset(object):
    ...
    def __init__(self, cfg):
        ...
        self.labels = np.zeros((3 * self.batch_size, 1))
    ...
    def initialize(self) -> np.ndarray:
        ...
        """
        # Create a batch of grids (each grid is zeros initially).
        if self.graph_type != "mixed":
            self.x0 = np.zeros((self.batch_size, self.N, self.N))
        else:
            self.x0 = np.zeros((3 * self.batch_size, self.N, self.N))
    ...
    def gen_static_graph_data(
        ...
        def solve_ode_single(x0_single, t_single, L_single, A_single):
        ...
        # Use jax.vmap to solve for all batch instances in parallel.
    ...
    def gen_sampling_time(self) -> np.ndarray:
        ...
        if self.sampling_type == "equal":
            ...
            if self.graph_type != "mixed":
                ...
                return np.tile(t, (3 * self.batch_size, 1))
            ...
            t_batch = []
            bs = 3 * self.batch_size if self.graph_type == "mixed" else self.batch_size
            for _ in range(bs):
    ...
    def vis(self, solution_numerical: np.ndarray) -> None:
        """
        Visualize the numerical solution of the ODE.

        This method visualizes the dynamic graph data by plotting the initial state and
        the solution at specified time points. The visualization is performed every 5 time steps.

        Parameters:
            solution_numerical (np.ndarray): The numerical solution of the ODE, with shape
                (batch_size, num_time_points, num_nodes, 1).
