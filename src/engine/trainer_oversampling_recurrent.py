...
def cross_entropy_loss(
    model: eqx.Module,
    data_i: tp.Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray],
    *,
    key: jr.PRNGKey,
    ...
    if isinstance(model, DCRNNModelSingleStep):
        out = jax.vmap(model)(x0_i, adj)
        pred_logits = jnp.squeeze(jnp.sum(out, axis=-2))
    else:
        out = jax.vmap(model, in_axes=(0, None))(x0_i, key)
        pred_logits = jnp.squeeze(jnp.sum(out, axis=-2))
...
class Trainer(pydantic.BaseModel):
    ...
    model: DCRNNCfg | ASTGCNCfg | StidGCNCfg = pydantic.Field(
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            train_loss, train_acc, model, opt_state, max_grad, max_update = make_step(
                model, optimiser, loss, data_i, opt_state, run_key
            )
            ...
            if (epoch + 1) % self.eval_freq == 0:
                ...
                val_loss, val_acc = validation_loss_fn(model, val_data_i, key=run_key)
                end_time = time.time()
                logger.info(
                ...
                if val_loss < best_validation_loss:
                    ...
                    test_loss, test_acc = validation_loss_fn(
                        model, test_data_i, key=run_key
                    )
...
def make_step(
    ...
    """
    (loss, acc), grads = eqx.filter_value_and_grad(loss, has_aux=True)(
        model, data_i, key=key
    )
