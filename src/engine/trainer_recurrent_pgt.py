...
from configs import *
from models import DCRNNModelSingleStep

logging.basicConfig(level=logging.INFO)


def mse_loss(
    model: eqx.Module, data_i: tp.Tuple[jnp.ndarray, jnp.ndarray], *, key: jr.PRNGKey
) -> jax.Array:
    """
    ...
    """
    x0_i, label_i, adj = data_i

    if isinstance(model, DCRNNModelSingleStep):
        pred_y = model(x0_i, adj)
        pred_y = jnp.squeeze(pred_y, axis=0)
    else:
        pred_y = model(x0_i, key)
...
class Trainer(pydantic.BaseModel):
    ...
    model: StidGCNCfg | ASTGCNCfg | DCRNNCfg = pydantic.Field(
    ...
    dataset: PGTDataSetCfg = pydantic.Field(..., description="Dataset configuration")
        ...
        for epoch in range(self.epochs):
            ...
            for train_batch_dict in train_data_loader:
                ...
                batch_train_loss, model, opt_state, batch_max_grad, batch_max_update = (
                    make_step(model, optimiser, loss, data_i, opt_state, run_key)
                )
            ...
            if (epoch + 1) % self.eval_freq == 0:
                ...
                for val_batch_dict in val_data_loader:
                    ...
                    validation_loss += loss(model, val_data_i, key=run_key)
                ...
                if validation_loss < best_validation_loss:
                    ...
                    if self.checkpoint_dir:
                        ...
                        for test_batch_dict in test_data_loader:
                            ...
                            test_loss += loss(model, test_data_i, key=run_key)
...
def make_step(
    ...
    """
    loss, grads = eqx.filter_value_and_grad(mse_loss)(model, data_i, key=key)
